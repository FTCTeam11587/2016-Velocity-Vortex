...
Tools that you'll need.
  git command line utilities    https://git-scm.com/downloads
  and whatever the ftc site says.
  
  it may also help to have the git GUI https://desktop.github.com/


  To run the command line interface (CLI) commands below, you could use the built in DOS command prompt (yet another open
application on your desktop taking up memory) or you could utilize the CLI built-in to android-studio.
  The Windows old-school CLI method:
        ...
        a mysterious black window, that you type in, will appear
        and youll need to "CD" to the project root directory.
        
  Android Studio CLI Method: (Recommended for Progrommers)
  
        At the very bottom of the android-studio project window, a little bit to the left,
          click "Terminal"
        And quite conveniently, you are already located in the project root directory
        
  There is also a Github "app", with buttons for the below procedures

How to update files.
  1.)  If you wanted to grab the latest and greatest source code, from the server, for your project:
  
              cd Robot      // Change Directories to the project root, or just make sure that you're somewhere in it.
              git pull      // tell git to pull the newest changes, that your teammates have made,
                            //    and pushed to the server.
                            
  2.)  If you wanted to push your latest and greatest source code, to the server, so your friends can
        see them, while you continue slaving away...:
        
              cd Robot      // Same as above.
              git add/rm SOMEFILENAME   // If you created a file, or deleted a file, we should tell git about it.
                            // But, if you didn't add or remove a file, and just editing existing files, then skip the
                            // git add or git rm (remove)
              git commit    // A commandline text editor will open with a bunch of text saying what files you
                            // edited, deleted, or added.  In that text editor, you'll need to write a comment
                            // on what you did.
              git push      // tell git to push the newest changes to the server.
              
  3.)  Repeat 1 and 2, as necessary
  
  
 If for some reason none of this is working correctly, I would love to help make it work!
 I can screen-share via facetime or hangouts.
    facetime: calc_man1@mac.com
    hangouts: hitchcme@gmail.com
 ...
 * Changes in the Android SDK, JDK and build tool requirements (minsdk=19, java 1.7, build tools 25.0.0).
